...
if [ ! -d "$SPHY_2D_MODULES" ]; then
    echo "Warning: Source directory not found: $SPHY_2D_MODULES"
fi

if [ ! -d "$LIM_MODULES" ]; then
    echo "Warning: Source directory not found: $LIM_MODULES"
fi
...
# Sync modules from sphy-2d/data/modules
if [ -d "$SPHY_2D_MODULES" ]; then
    echo "Syncing modules from: $SPHY_2D_MODULES"
    for module_dir in "$SPHY_2D_MODULES"/*; do
        if [ -d "$module_dir" ]; then
            module_name=$(basename "$module_dir")
            sync_module "$SPHY_2D_MODULES" "$module_name"
        fi
    done
fi
...
if [ -d "$LIM_MODULES" ]; then
    echo "Syncing modules from: $LIM_MODULES"
    for module_dir in "$LIM_MODULES"/*; do
        if [ -d "$module_dir" ]; then
            module_name=$(basename "$module_dir")
            sync_module "$LIM_MODULES" "$module_name"
        fi
    done
fi
